...
lift_type lift_create(void)
{
  ...
  {
    pthread_cond_init(&lift->change_floor[i],NULL);
  }
  pthread_cond_init(&lift->passenger_change,NULL);
  pthread_cond_init(&lift->lift_change,NULL);
...
// Persons that will leave on this floor
static int persons_about_to_leave_lift(lift_type lift){
...
void lift_has_arrived(lift_type lift)
{
  ...
  pthread_cond_broadcast(&lift->lift_change);
  pthread_cond_broadcast(&lift->change_floor[lift->floor]);

  // Wait for everybody to leave and enter
  while( !(persons_about_to_leave_lift(lift) == 0 && (n_passengers_in_lift(lift) == MAX_N_PASSENGERS || persons_on_floor(lift,lift->floor) == 0)) ) {
    pthread_cond_wait(&lift->passenger_change, &lift->mutex);
  }
...
/* passenger_wait_to_leave: returns non-zero if the passenger shall
wait for the lift to get to their to_floor, otherwise returns zero */
static int passenger_wait_to_leave(lift_type lift, int to_floor)
{
  int waiting_ready =
  /* the lift is not moving */
  !lift->moving &&
  /* and the lift is at to_floor */
  lift->floor == to_floor;
...
static void enter_floor(
  ...
  static void leave_floor(
    ...
    void lift_travel(lift_type lift, int id, int from_floor, int to_floor)
    {
      ...
      // Wait for lift to arrive on my floor
      while(passenger_wait_for_lift(lift, from_floor)) {
        pthread_cond_wait(&lift->change_floor[from_floor], &lift->mutex);
      }
      // Enter lift
      leave_floor(lift, id, from_floor);
      enter_lift(lift,id, to_floor);

      pthread_cond_broadcast(&lift->passenger_change);

      // Wait until lift (I am in lift) get to the right floor
      while(passenger_wait_to_leave(lift, to_floor)) {
        pthread_cond_wait(&lift->lift_change, &lift->mutex);
      }

      // Leave lift and enter floor
      leave_lift(lift, id);
      //enter_floor(lift,id,to_floor);
      pthread_cond_broadcast(&lift->change_floor[lift->floor]); //Tell others that they can start working!
      pthread_cond_broadcast(&lift->passenger_change);
      pthread_mutex_unlock(&lift->mutex);
    }
